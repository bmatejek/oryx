...
def ConnectBackground(segmentation, max_label):
    # everything needs to be ints to work with c++
...
def FindBackgroundNeighbors(segmentation, max_label):
    # everything needs to be ints to work with c++
    assert (segmentation.dtype == np.int32)
    ...
    # call the c++ function
    CppFindBackgroundNeighbors(&(cpp_segmentation[0,0,0]), &(cpp_grid_size[0]), max_label)
